    ...
    * 게시판 메인 페이지에서 등록하기 버튼을 선택 후 팝업창이 표시되고, 이 곳에서 작성자, 비밀번호 작성 후 확인버튼을 누르면 게시글 등록이 가능한 글쓰기 페이지로 이동 가능함
    ...
    <게시판 메인 페이지에서 글쓰기 버튼을 누르고 들어온 경우의 화면>
      * 비밀번호 생성 조건
        ...
        * 비밀번호 입력 표시는 보안상 한 문자당 '*' 형태로 표시되어야 함
...
| pk, fk 여부  | name     |      type     |
|---------|---------------|--------------|
|   **pk**| boardIdx      |     int(auto increment)    |
|         | boardTitle    | varchar(255)  |
|         | createDt      | DATETIME     |
|         | updateDt      | DATETIME     |
|         | boardContent  | varchar(255)||         | boardIsDelete  |      boolean|
|   **fk**| memberIdx      | int          |  
|   **fk**| attachmentIdx  | int          |
|**fk**         | commentIdx     | int |   
...
|---------|----------------|--------------|
|    **O**    | attachmentIdx  |int       |
|         | attachmentIdDelete | boolean  |
|         | attachmentNickname | varchar(255)|
|         | attachmentUrl|   varchar(255)|
|      | attachmentlength |   varchar(255)|
...
|         | commentCreateDt | datetime|
|    | commentUpdateDt |datetime|
|         | commentIsDelete|   varchar(255)|
|      | commentPw |   varchar(255)|
| **fk** | memberIdx | int |
